...
// Motion One のアニメーションスタイル型（型チェック回避用）
// biome-ignore lint/suspicious/noExplicitAny: Motion One の型定義が不完全なため
type AnimationStyle = Record<string, any>;
...
async function animateStep(): Promise<void> {
	...
	if (step.found) {
		await animate(
			...
			{ duration: 0.4, easing: "ease-out" } as AnimationStyle,
		).finished;
...
function resetArrayVisuals(): void {
	const boxes = document.querySelectorAll<HTMLElement>(".array-box");
		...
		if (elements.infoResult) {
			if (step.found) {
				elements.infoResult.className =
					"px-6 py-3 bg-green-100 text-green-700 rounded-lg text-lg";
				elements.infoResult.textContent = "✓ 見つかりました！";
			} else {
				elements.infoResult.className =
					"px-6 py-3 bg-neutral-100 text-neutral-600 rounded-lg";
		...
		if (elements.resultMessage && elements.resultText) {
			...
			if (state.foundIndex !== undefined) {
				elements.resultMessage.className =
					"p-6 rounded-xl border-2 mt-4 bg-green-50 border-green-300";
				...
				elements.resultMessage.className =
					"p-6 rounded-xl border-2 mt-4 bg-red-50 border-red-300";
